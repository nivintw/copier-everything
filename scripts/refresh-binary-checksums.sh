...
# current *_VERSION. Renovate bumps the version env vars (see .github/renovate.json5), but the
...
tool_version_at_base() { # <TOOL> <baseref> -> the tool's *_VERSION at base (any file) or empty
  ...
  files="$(git grep -lE "^[[:space:]]*${tool}_VERSION: \"" "$baseref" -- \
    '.github/workflows/*.yml' '.github/workflows/*.yaml' 'template/.github/workflows/*.jinja')" && rc=0 || rc=$?
  if [ "$rc" -gt 1 ]; then
    echo "ERROR: tool_version_at_base: git grep failed (exit $rc) for ${tool} at ${baseref}" >&2
    exit 1
  fi
  [ -n "$files" ] || return 0
...
refresh_pin() { # <file> <TOOL> <suffix> <fetch_fn> <valid-regex> <label>
  ...
  local version old_pin new_pin base_version tmp
  ...
  if [ -n "$BASE_REF" ] && [ "$new_pin" != "$old_pin" ]; then
    base_version="$(tool_version_at_base "$tool" "$BASE_REF")"
