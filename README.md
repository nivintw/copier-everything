...
| **Renovate** (`.github/renovate.json`) | Automates the pins (pre-commit hook revs + action digests) and groups `ruff` bumps so a new lint rule lands as a reviewable PR, not a surprise red. An `approve-bot-prs.yml` workflow auto-approves the repo's own bot PRs (Renovate, release-please) so those unattended flows survive a ruleset that requires approving reviews — approval isn't merge; required checks still gate (self-disabling without App credentials). |
